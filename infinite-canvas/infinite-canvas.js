    ...
    function initializeWorld(ctx) {
        ...
        infinity.updateChunks = function () {
            ...
            Object.keys(chunks).forEach(function (key) {
                ...
                // top-left
                var A = {
                    x: Math.constrain(renderCoord.x, 0, canvas.width),
                    y: Math.constrain(renderCoord.y, 0, canvas.height)
                };
                // bottom-right
                var D = {
                    x: Math.constrain(renderCoord.x + configuration.chunkWidth,  0, canvas.width),
                    y: Math.constrain(renderCoord.y + configuration.chunkHeight, 0, canvas.height)
                };
                ...
                offscreenRenderCtx.drawImage(canvas, A.x, A.y, width, height, putLocation.x, putLocation.y, width, height);
        ...
        infinity.moveBy = function(dx, dy, canvas) {
